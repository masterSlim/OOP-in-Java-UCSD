	...
	private void printQuakes() 
	{
		...
		for (Marker q : quakeMarkers) {
			if(q.getProperty("country") != null) {
					if(!(eqCounter.containsKey(q.getProperty("country")))) {
						//если в счётчик землятресений страна ещё не добавлена
						eqCounter.put((String)q.getProperty("country"), 1);
						//(new Feature() (q.getProperty("country"), 1));
	
					}else {
						//eсли в счётчик страна уже добавлена 
						int c = eqCounter.get(q.getProperty("country")) + 1;
						eqCounter.put((String) q.getProperty("country"), c);
					}
			} else {
				// если землятресение произошло в океане (вне стран)
				oceanCounter += 1;
			}
		}
		for(Map.Entry<String, Integer> entry : eqCounter.entrySet()) {
