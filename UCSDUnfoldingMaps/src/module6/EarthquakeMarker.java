...
public abstract class EarthquakeMarker extends CommonMarker implements Comparable<EarthquakeMarker>
{
	...
	public int compareTo (EarthquakeMarker marker) {
		...
		/*
		 * int var; if(this.getMagnitude() < marker.getMagnitude()) { var = -1;
		 * System.out.println(this.getMagnitude() + " second: " + marker.getMagnitude()
		 * + var); }else { if(this.getMagnitude() > marker.getMagnitude()) { var = 1;
		 * System.out.println("Else: " + this.getMagnitude() + " second: " +
		 * marker.getMagnitude()); } else { var = 0; } } return var;
		 */
	}
	...
	private void colorDetermine(PGraphics pg) {
		...
		if(depth >= THRESHOLD_DEEP){
			...
			//adjusting earthquake depth(0-300) to color range (0-255)
			colorLevel = (int) (depth / 1.1765);
			//System.out.println("depth: " + depth + "colorLevel: " + colorLevel);
		} 
				
		pg.fill(255-colorLevel, 40, colorLevel);
						
		/*
		 * if (depth < THRESHOLD_INTERMEDIATE) { pg.fill(255, 255, 0); } else if (depth
		 * < THRESHOLD_DEEP) { pg.fill(0, 0, 255); } else { pg.fill(255, 0, 0); }
		 */
	}
